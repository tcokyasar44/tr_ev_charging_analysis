...
\Cref{fig:detour_overhead} illustrates the relationship between trip distance and charging detour overhead for the 350~kW, 50-plug scenario. Short trips (under 100~km) incur proportionally higher detour overhead because a single charging stop represents a larger fraction of the total distance. As trip distance increases, the detour percentage declines and stabilizes, reflecting the amortization of a roughly fixed number of detour events over a longer route. The scatter also reveals substantial trip-level heterogeneity: at any given distance, detour overhead varies by a factor of 5--10 depending on the specific origin-destination pair and the proximity of the route to charging stations.
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.85\linewidth]{plot4_detour_overhead_350kW_50plugs.png}
\caption{Charging detour as a percentage of original trip distance for the 350~kW, 50-plug scenario. Each point represents one feasible path (5,000 randomly sampled for visibility). Shorter trips incur proportionally higher detour overhead.}
